...
작성일: 2026-09-16 · 버전: 1.3

## 1. 서비스 범위

써랍은 신입 취업 준비생이 기본 이력과 경험을 정리하고, 회사별 자소서를 직접 작성할 때 재사용하는 개인 기록 서비스다. 인턴 및 기타 근무 경력을 포함한다. 자소서 답변 자동 생성은 제공하지 않는다.

이 문서는 ① 엔터티·필드·관계·삭제 규칙 ② DBML ③ 화면별 API와 요청·응답 규칙을 정의한다. 화면, API, 데이터 모델은 기본 이력 관리부터 자소서 검토 메모 확인까지 동일한 사용자 흐름을 따른다.
...
1. 16개 테이블로 구성한다. 단순 분류마다 테이블을 추가하지 않고, 여러 건을 관리하거나 실제 관계를 갖는 정보만 분리한다.
...
7. 등록된 사용자 중 하나를 선택해 로그인한다. 소유자 ID는 로그인 세션에서 결정하고 일반 쓰기 요청 body에 넣지 않는다.
8. 작성 완료 자소서만 다른 사용자에게 검토를 요청할 수 있다. 검토자는 본문을 수정하지 못하며 문항별 메모 1개를 작성·수정하고, 완료된 메모만 작성 화면 자료 서랍에 노출한다.
9. 친구 관계, 링크 공유, 이메일·푸시·실시간 알림, 인라인 문장 댓글, 답글은 범위 밖이다. 받은 REQUESTED 요청 수를 화면 배지로 사용한다.
10. 경험 추천은 문항과 본인 경험의 내용·태그를 비교한다. 응답의 mode 값으로 TAG_MATCH와 AI 추천 방식을 구분하며 자소서 답변 자체는 생성하지 않는다.
...
| profiles | 병역·보훈·장애 및 추가 정보 | users와 1:0..1 |
| educations | 학력·학점 | 사용자 1:N |
| courses | 학력별 수강 과목 | 학력 1:N |
...
| review_requests | 작성자–검토자 검토 요청 | 자소서 1:N, 사용자별 보낸·받은 요청 |
| review_comments | 문항별 검토 메모 | 검토 요청 1:N, 문항·검토자 연결 |

```mermaid
erDiagram
    users ||--o| profiles : owns
    users ||--o{ educations : owns
    educations ||--o{ courses : contains
    ...
    applications ||--o{ review_requests : receives
    users ||--o{ review_requests : requests
    users ||--o{ review_requests : reviews
    review_requests ||--o{ review_comments : contains
    essay_questions ||--o{ review_comments : receives
...
| 수강 학기 1 / 2 / 여름 / 겨울 | FIRST_SEMESTER / SECOND_SEMESTER / SUMMER / WINTER |
| 과목 유형 전공 / 교양 / 기타 | MAJOR / GENERAL / OTHER |
| 재수강 N / Y | false / true |
| 검토 대기 / 검토 완료 | REQUESTED / COMPLETED |
...
- 화면에서 `해당 없음`을 선택하면 군별·계급·복무 기간을 즉시 비활성화하고 기존 값을 지운다. 서버도 NOT_APPLICABLE 상태에 세부값이 전달되면 400으로 응답한다.
...
- CRLF/CR을 LF로 정규화한 후 Unicode code point 수를 센다. 공백·줄바꿈을 포함하며 Java 서버는 codePointCount, Vue는 Array.from(answer).length를 사용한다.
...
- POST /auth/login은 등록된 사용자 ID를 받아 로그인 세션을 시작하고 현재 사용자 정보를 반환한다.
- 검토 요청자는 자소서 소유자여야 하고 검토자는 요청자와 달라야 한다. 자소서가 COMPLETED일 때만 요청하며 같은 자소서·검토자에 REQUESTED 요청은 하나만 허용한다.
- 검토자는 자신에게 온 요청만 읽고 문항별 메모를 작성한다. applications와 essay_questions의 수정 API에는 접근 권한이 없다.
- 메모는 1~5,000자이며 `(review_request_id, question_id, author_id)`로 문항별 1개를 유지한다. REQUESTED에서만 저장·삭제할 수 있다.
- 메모가 하나 이상 있어야 검토를 COMPLETED로 바꿀 수 있다. 완료 후 메모는 잠기며 작성자는 COMPLETED 메모만 작성 화면에서 조회한다.
- 검토 진행 중에는 대상 자소서를 DRAFT 또는 SUBMITTED로 바꾸지 않는다. 검토 완료 후 작성자가 DRAFT로 되돌려 수정할 수 있다.
...
| 지원 건 삭제 | 문항·참고 경험 연결·검토 요청·메모 삭제, 경험 원본 유지 | 지원 건 + 문항 + 연결 + 검토 |
...
| 자격증·어학 삭제 | 해당 항목만 삭제 | 단일 항목 |
| 학력 삭제 | 연결된 수강 과목 함께 삭제 | 학력 + 수강 과목 |
| 검토 요청 생성 | 자소서·요청자·검토자·상태 검증 후 요청 생성 | 단일 검토 요청 |
| 검토 메모 저장 | 요청·문항·검토자 권한 검증 후 문항별 upsert | 단일 검토 메모 |
| 검토 완료 | 메모 존재 확인 후 상태·완료 시각 갱신 | 단일 검토 요청 |

- 모든 삭제는 하드 삭제다. UI에 확인창을 둔다. 휴지통·복구는 범위 밖.
- 사용자 삭제 API는 제공하지 않는다. users의 cascade는 사용자 데이터 정리 시 참조 무결성을 유지한다.
...
- 파일 삭제/외부 API/AI 호출을 CRUD 트랜잭션에 포함하지 않는다. 자동 저장은 마지막 입력 후 2초 동안 추가 변경이 없을 때 현재 초안을 덮어쓰는 디바운스 방식이며 버전을 생성하지 않는다. 화면 이동 시에는 즉시 저장한다. 한 자소서의 동시 편집은 허용하지 않는다.
...
| 로그인 | users | POST /auth/login, POST /auth/logout |
| 기본 이력 | profiles + 학력/수강 과목/자격증/어학 배열 | GET /me/resume, PUT /me/profile, 하위 항목 CRUD |
| 경력 | careers | /careers CRUD |
| 경험 | experiences + tags | /experiences CRUD, GET /tags |
| 보관함 | applications | /applications CRUD, q/status 필터 |
| 자소서 작성 | applications + essay_questions + question_versions + review_comments | GET /applications/{id}, 문항 CRUD, 완료·상태·버전 API, GET /applications/{id}/review-feedback |
| 참고 경험 패널 | experiences + question_experiences | 경험 목록/상세, 연결 PUT/DELETE |
| 받은·보낸 검토 요청 | review_requests + review_comments | 검토 요청 목록·상세·메모·완료 API |
| 추천 패널 | 경험·태그 | POST /experience-recommendations |

목록은 개인 자료 규모를 전제로 전체 배열을 반환한다. 수정일 내림차순, id 내림차순이며 문항만 position 오름차순이다. 빈 결과는 200 + []이며 검색 실패로 처리하지 않는다.

기본 이력의 저장·수정 전환은 API 성공 여부를 표현하는 화면 상태이며 별도 DB 필드가 아니다. 수강 과목 다운로드는 조회된 데이터를 브라우저에서 CSV 파일로 변환하므로 별도 API를 사용하지 않는다.
